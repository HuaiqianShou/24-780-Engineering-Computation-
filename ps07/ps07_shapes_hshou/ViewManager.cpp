...
void ViewManager::load()
{
	...
	if (inFile.is_open()) {
		delete theShape;
		// cout << (inFileName.find("Lshape") == string::npos) << endl;
		// here if it is Lshape file then we create a Lshape class, if it is a Ishape file we create a 
		/// Ishape class, otherwise we create a shape class
		if (inFileName.find("Lshape") != string::npos|| inFileName.find("lshape") != string::npos) {
			theShape = new Lshape();
		}
		else if (inFileName.find("Ishape") != string::npos || inFileName.find("ishape") != string::npos) {
...
bool ViewManager::manage()
{
	...
	case FSKEY_P: 
		shapeColor = (shapeColor + 240 / 8) % 240;
		break;
	...
	// show properties
	if (showProperties) {
		vector<string> allProps = theShape->getProperties();
		int currLine = 1;
		glColor3ub(255, 255, 255);
		DrawingUtilNG::drawRectangle(0, 0, 152, 68, true);
		glColor3ub(255, 0, 0);
		for (auto& currProp : allProps) {
			glRasterPos2d(2, currLine * 12);
			YsGlDrawFontBitmap7x10(currProp.c_str());
			currLine++;
		}
	}
...
void ViewManager::showMenu()
{
	...
	cout << "    Q : show/hide shape properties" << endl;
